...
TODO
    - [ ]   add set_frame() method to add a dataframe after construction
            all checks on column validation will either be done each method call (current)
                or all at once when set_frame() is done (validates that the columns pre-set are valid)
...
class DataFrameAggregator():
    ...
    def __init__(self, frame: pd.DataFrame=None):
        ...
        Args
        ----
        df: Optional. The dataframe to aggregate, if omitted the DataFrameAggregator validates itself
    ...
    def __repr__(self):
        return f"DataFrameAggregator on {self.frame}\n"
    ...
    def set_frame(self, frame: pd.DataFrame) -> None:
        ...
        """
        preset_columns = list(set(self.group_cols + [specs.column for specs in self.measures.values()]))
        frame_columns = frame.columns
        for col in preset_columns:
            if col not in frame_columns:
                raise ValueError(f"{col} was set to be aggregated but is not part of frame!")
        self.frame = frame
    ...
    def add_subtotal(self, column: str, alias: str, subtotal_filter: list, subtotal_incl: bool=True) -> None:
        ...
        self.add_groupby_column(column)
        if self._alias_is_unique_(alias):
            self.subtotals[alias] = _TotalColSpecs(column, subtotal_filter, subtotal_incl)
    ...
    def aggregate(self, totals_only: list=None) -> pd.DataFrame:
        ...
        if totals_only:
            ...
            for col in totals_only:
                if col not in cols_in_totals:
                    raise ValueError(f"{col} in 'totals_only' does not have a grand/sub total set. Expected one of {cols_in_totals}")
            ...
            comb_min = 1
            all_stats.append(self._compute_measures_(self.frame, self.group_cols, self.measures))
        ...
        for r in range(comb_min, comb_max + 1):
            for subset in combinations(all_totals.keys(), r):
                ...
                subset_columns = {alias: specs for alias, specs in all_totals.items() if alias in subset}
                ...
                parent_columns_distinct = len(parent_columns) == len(set(parent_columns))
                ...
                if parent_columns_distinct and has_all_total_cols:
                    ...
                    grandtotals = {specs.column: alias for alias, specs in subset_columns.items() if specs.sub_values is False}
                    df = self.frame.transform(self._apply_alias_transform_, 'index', to_transform=grandtotals)
                    
                    # subtotals first filter the column then transform all values to the aliases
                    subtotals = {alias: specs for alias, specs in subset_columns.items() if isinstance(specs.sub_values, list)}
                    for specs in subtotals.values():
                        df = df[df[specs.column].isin(specs.sub_values) == specs.incl]
                    df = df.transform(self._apply_alias_transform_, 'index', to_transform={specs.column: alias for alias, specs in subtotals.items()})
                    
                    # measures can now be calculated on the dataframe as it has been adjusted for grand/sub totals
                    all_stats.append(self._compute_measures_(df, self.group_cols, self.measures))
    ...
    @staticmethod
    def _compute_measures_(frame: pd.DataFrame, group_cols: list, measures: dict) -> pd.DataFrame:
        ...
        """        
        agg_dict = {specs.column: specs.aggfunc for specs in measures.values()}
    ...
    def _alias_is_unique_(self, alias: str) -> bool:
        ...
        """
        if any([
            alias in self.grandtotals,
            alias in self.subtotals,
            alias in self.measures
        ]):
            raise KeyError(f"{alias} already in use! Duplicate aliases are not supported.")
        return True
